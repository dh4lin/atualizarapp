...
def check_apis_for_app(appid: int) -> str:
    ...
    apis = load_api_manifest() or [] # Garante que seja uma lista
    
    # Adicionamos a sua Skyapi manualmente na lista de busca se ela não existir
    if not any(api.get("name", "").lower() == "skyapi" for api in apis):
        apis.append({
            "name": "Skyapi",
            "url": f"https://github.com/skyflarefox/Skyapi/raw/refs/heads/main/{appid}.zip",
            "success_code": 200
        })
    ...
    # Tenta o check rápido do servidor externo
    ...
    for api in apis:
        ...
        # Processa a URL (substitui chaves se existirem)
        if "<moapikey>" in template:
            if not morrenus_api_key: continue
            template = template.replace("<moapikey>", morrenus_api_key)
        
        url = template.replace("<appid>", str(appid))
        available = False
        
        # 1. Tenta validar pelo fast_check
        ...
        # 2. Se não validou no fast_check, faz a verificação manual (HEAD/GET)
        if not available:
            try:
                if name.lower() == "morrenus":
                    ...
                    # Tenta HEAD primeiro para poupar banda (especialmente no GitHub)
                    resp = client.head(url, headers=headers, follow_redirects=True, timeout=5)
                    if resp.status_code == success_code:
                        available = True
                    elif resp.status_code in [405, 403]: # Se proibido ou método não aceito, tenta GET
